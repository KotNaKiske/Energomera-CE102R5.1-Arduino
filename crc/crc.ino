uint8_t checksum;
const unsigned char crc8tab[256] = {   };

void setup() {
  // put your setup code here, to run once:
  Serial.begin(115200);
}

void loop() {
  byte CmdEtope[] = {0x48, 0x69, 0xD5, 0xFD, 0x00, 0x31, 0xDE, 0x0B, 0x00, 0xD0, 0x01, 0x32};//запрос показаний
  checksum = CRC8(CmdEtope, 12);
  Serial.println("CRC8 checksum:");
  Serial.println(String(checksum, HEX));
  delay(2000);
}

byte CRC8(const byte *Buffer, byte Length) {
  byte crc8;
  crc8 = 0; for (int i = 0; i < Length; i++) {
    crc8 = crc8tab[crc8 ^ Buffer[i]];
  }
  return crc8;
}
